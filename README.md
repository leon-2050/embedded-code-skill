...
> 嵌入式 C 的 REWRITE/REVIEW/GUIDE：四层整理、低层审查、RTOS/构建咨询。硬件参数须有出处。
...
- **REWRITE**：整理旧驱动代码，保留寄存器写入顺序和时序；按四层架构标准化命名与文件组织
...
flowchart TB
    ...
    subgraph DRV["驱动层  module_drv.h / module_drv.c"]
        ...
        D2["ISR 通过回调通知应用层"]
        D_ban["✗ 不含业务逻辑  ✗ 不直触寄存器结构体"]
    end

    subgraph LL["访问层  module_ll.h / module_ll.c"]
        direction TB
...
**文件布局**：每外设七个文件 —— `module_reg.h` + `module_ll.h/.c` + `module_drv.h/.c` + `module.h/.c`。
平铺项目不建 `module/` 子目录，七文件直接平铺在当前目录。带 `module/` 子目录的项目布局相同，仅多一层目录。
...
| P1 结构类 | 四层拆分粒度、文件组织 | 项目已有既定架构时让位 |
